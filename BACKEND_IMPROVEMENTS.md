...
- [x] **Create Bulk Clear & Single Tenant Reset API (`DELETE /api/v1/admin/reset`)**: Endpoint implemented to clear/reset tenant resource data and staff while strictly preserving the primary Super Admin user account. Accepts `{ "id": 105 }` for single tenant reset or `{ "clear_all": true }` to wipe test data across all tenants.
- [x] **Super Admin Tenant Workspace Switching (`X-Tenant-ID`)**: When `X-Tenant-ID` header is passed by Super Admin, all queries and creations (leads, bookings, packages, inventory, finance) are automatically scoped to that company ID. `config/cors.php` updated with `X-Tenant-ID` in `allowed_headers`.
...
- **Payload**: `role_type` parameter is now optional.
